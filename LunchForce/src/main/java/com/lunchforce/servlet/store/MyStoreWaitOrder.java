...
/**
 * 수락 대기중인 주문 리스트를 가져오는 서블릿
 */
@WebServlet("/store/MyStoreWaitOrder")
public class MyStoreWaitOrder extends HttpServlet {
	@Override
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			int resultCount = orderDAO.getStoreOrderDTOListCount(storeDTO.getStoreId(), 0);
			// 2. 수락 대기중인 주문의 페이징된 리스트를 가져옴
			ArrayList<OrderDTO> orderList = orderDAO.getStoreOrderDTOListpaging(storeDTO.getStoreId(), 0, page,
					pageCount);
			// 3. request에 넣어줌
		...
		// 페이지 이동
		request.getRequestDispatcher("../store/MyStoreWaitOrder.jsp").forward(request, response);
		return;
	}
}
