 ...
 * 수락한 주문을 완료시키는 서블릿
 */
@WebServlet("/store/MyStoreOrderComplete")
public class MyStoreOrderComplete extends HttpServlet {
	@Override
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			//1. 주문 완료 수행
			OrderDAO.getInstance().setStoreOrderState(storeDTO.getStoreId(), orderId, 2, time);
		} catch (Exception e) {
			e.printStackTrace();
		}
		
		//2. 수락한 주문 보는 페이지로 이동
		response.sendRedirect("../store/MyStoreAcceptOrder");
		return;
	}
}
